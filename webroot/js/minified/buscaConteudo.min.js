...
document.querySelectorAll(".pwd").forEach(function(currentValue,currentIndex,listObj){currentValue.addEventListener("change",estaComprometida)});let buscaUserPass=async function(event){let button=event.target;if(button.tagName=='I'){button=button.parentElement}
let body={'type':button.getAttribute('data-clipboard-tipo'),'id':button.getAttribute('data-clipboard-entrada-id')};let urlParaBusca=window.location.origin+'/entradas/clipboard/';try{if(navigator.clipboard&&navigator.clipboard.writeText){await navigator.clipboard.writeText(await factoryRequest(urlParaBusca,{'body':JSON.stringify(body)}).then((response)=>{if(!response.ok){throw new Error(response.status+' - '+response.statusText)}
return response.json()}).then(function(dadoRetornado){let tooltip=new bootstrap.Tooltip(button,{'trigger':'manual','title':body.type=='user'?'Usuário copiado':'Senha copiada',});tooltip.show();setTimeout(()=>{tooltip.hide();setTimeout(()=>{tooltip.dispose()},300)},2000);return dadoRetornado.data}))}else{alert('Funcionalidade não suportada'+'\n\nSeu navegador não permite cópia automática para área de transferência.'+'\n\nSolução: navegue até a entrada desejada e copie o conteúdo usando Ctrl+C.')}}catch(error){console.error(error);alert('Erro ao copiar para área de transferência.'+'\n\nSolução: navegue até a entrada desejada e copie o conteúdo usando Ctrl+C.'+'\n\nDetalhes técnicos: '+error.message)}}
document.querySelectorAll(".btn-clipboard").forEach(function(currentValue,currentIndex,listObj){currentValue.addEventListener("click",buscaUserPass)})