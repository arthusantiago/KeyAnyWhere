...
function criarItemListaBusca(texto,href){let item=document.createElement('li');let link=document.createElement('a');link.textContent=texto;if(href){link.setAttribute('href',href)}
item.appendChild(link);return item}
function substituirConteudoLista(idElemento,itens){let container=document.getElementById(idElemento);while(container.firstChild){container.removeChild(container.firstChild)}
itens.forEach(function(item){container.appendChild(item)})}
let paraExecutar;function buscaGenerica(event){let inputOrigemBusca=event.target;let destinoHtmlRetorno=inputOrigemBusca.getAttribute('data-busca-inserir-resultado');let urlParaBusca=inputOrigemBusca.getAttribute('data-busca-url');let config={'quantMinCaracter':3,'tempoEspera':2000};if(inputOrigemBusca.value.length>=config.quantMinCaracter){clearTimeout(paraExecutar);let chamadaAoServidor=function(){if(inputOrigemBusca.value.length>=config.quantMinCaracter){let dadosParaRequest=JSON.stringify({stringBusca:inputOrigemBusca.value,});factoryRequest(urlParaBusca,{'body':dadosParaRequest,}).then((response)=>{if(!response.ok){throw new Error(response.status+' - '+response.statusText)}
return response.json()}).then(function(resultado){if(!resultado.length){substituirConteudoLista(destinoHtmlRetorno,[criarItemListaBusca('Não localizado')]);return}
let itens=resultado.map(function(entrada){return criarItemListaBusca(entrada.titulo,entrada.url)});substituirConteudoLista(destinoHtmlRetorno,itens)}).catch(function(error){alert('Ocorreu um erro \n\n'+error.message)})}};paraExecutar=setTimeout(chamadaAoServidor,config.tempoEspera);substituirConteudoLista(destinoHtmlRetorno,[criarItemListaBusca('Buscando...')])}else{substituirConteudoLista(destinoHtmlRetorno,[criarItemListaBusca('Digite no mínimo '+config.quantMinCaracter+' caracteres')],)}}
...
function obterQrCode2FA(event){let body=JSON.stringify({idUser:event.target.getAttribute('data-qrcode-user-id'),novoQrCode:event.target.getAttribute('data-qrcode-novo')?!0:!1});let parametros={'headers':{'Accept':'text/plain'},'body':body,};let urlParaBusca=window.location.origin+'/users/geraQrCode2fa/';factoryRequest(urlParaBusca,parametros).then((response)=>{if(!response.ok){throw new Error(response.status+' - '+response.statusText)}
return response.text()}).then(function(dataUriQrCode){let container=document.getElementById('imagemQrCode');while(container.firstChild){container.removeChild(container.firstChild)}
let img=document.createElement('img');img.src=dataUriQrCode;img.alt='QR Code 2FA';container.appendChild(img)}).catch(function(error){alert('Ocorreu um erro \n\n'+error.message)})}
